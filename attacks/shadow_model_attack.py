...
#     target (≈44 K samples).
#  3. Train NUM_SHADOWS shadow models (same SmallCNN architecture).
...
#   outputs/logs/shadow_mia_results.csv
#   outputs/logs/shadow_mia_per_sample.csv
# =============================================================================
...
TARGET_CKPT   = OUT_DIR / "checkpoints" / "best_baseline.pt"
SHADOW_DIR    = OUT_DIR / "checkpoints" / "shadow_models"
ATTACK_DIR    = OUT_DIR / "checkpoints" / "attack_models"
...
class SmallCNN(nn.Module):
    """Lightweight 4-layer CNN for CIFAR-10 (32×32 RGB, 10 classes)."""
...
def train_one_shadow(
    ...
    opt   = optim.SGD(
        ...
        weight_decay=0,   # was 5e-4 — removed to encourage overfitting like the target
    )
    sched = optim.lr_scheduler.MultiStepLR(
        opt, milestones=[int(epochs * 0.6), int(epochs * 0.8)], gamma=0.5
    )
...
def evaluate_attack(attack_models, target_idx, holdout_idx):
    ...
    csv_path = RESULTS_DIR / "shadow_mia_results.csv"
    ...
    per_sample_path = RESULTS_DIR / "shadow_mia_per_sample.csv"
